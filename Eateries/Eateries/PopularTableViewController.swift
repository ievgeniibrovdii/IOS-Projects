...
class PopularTableViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        // refreshControl для догрузки ресторанов из iCloud
        ...
        spinner.color = #colorLiteral(red: 0.4666666687, green: 0.7647058964, blue: 0.2666666806, alpha: 1)
        spinner.translatesAutoresizingMaskIntoConstraints = false // xcode не будет проставлять констреинты сам (только мы должны)
    ...
    func getCloudRecords() {
        ...
        //        let predicate = NSPredicate(value: true) // порядок в котором приходят данные
        ...
        let query = CKQuery(recordType: "EateriesClass", predicate: predicate)
        ...
        queryOperation.queryCompletionBlock = { (cursor, error) in
            guard error == nil else {
                print("Не удалось получить записи с iCloud: \(error?.localizedDescription)")
                return
            }
            print("Records fetching from iCloud was succesed!")
            DispatchQueue.main.async {
                self.tableView.reloadData()
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let imageURL = cache.object(forKey: restaurant.recordID) as? URL {
            ...
            fetchRecordsOperation.perRecordCompletionBlock = { (record, recordID, error) in
                ...
                guard error == nil else {
                    print("Unposible to load image from iCloud!")
                    return
                }
